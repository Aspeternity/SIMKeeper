...
function occurrenceKey(reminderKey: string, dueDate: string | null) {
...
export function listReminderActions(limit = 100) {
  ...
  const rows = sqlite
    .prepare(
      `SELECT id, reminder_key, sim_id, sim_label, kind, title, due_date, action, snooze_until, acted_at
  ...
  const rows = sqlite
    .prepare(
      `SELECT ra.id, ra.reminder_key, ra.sim_id, ra.sim_label, ra.kind, ra.title,
              ra.due_date, ra.action, ra.snooze_until, ra.acted_at
...
export function filterReminderItems(reminders: ReminderItem[], today: string) {
  ...
  const latest = new Map(
    listLatestOccurrenceActions().map((action) => [occurrenceKey(action.reminderKey, action.dueDate), action]),
  );

  return reminders.filter((reminder) => {
    const action = latest.get(occurrenceKey(reminder.key, reminder.dueDate));
    if (!action) return true;
    if (action.action === "completed" || action.action === "ignored") return false;
    if (action.action === "snoozed" && action.snoozeUntil && today < action.snoozeUntil) return false;
    return true;
  });
}
  ...
  const result = sqlite
    .prepare(
      `INSERT INTO reminder_actions
       (reminder_key, sim_id, sim_label, kind, title, due_date, action, snooze_until, acted_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .run(
      input.reminder.key,
