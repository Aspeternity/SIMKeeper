...
export const COUNTRY_REGION_BY_CODE = new Map(
  COUNTRY_REGIONS.map((item) => [item.code, item] as const),
);

export function getCountryRegion(code: string | null | undefined) {
  if (!code) return null;
  return COUNTRY_REGION_BY_CODE.get(code.trim().toUpperCase()) ?? null;
}
