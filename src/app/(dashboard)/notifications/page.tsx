...
type ReminderKind = "sim_validity" | "keep_alive" | "low_balance";
...
function expandLegacyFilters(kinds: ReminderKind[], statuses: ReminderStatus[]): ChannelFilter {
  const legacyKinds = kinds.length === LEGACY_KINDS.length && LEGACY_KINDS.every((value) => kinds.includes(value));
  const legacyStatuses = statuses.length === LEGACY_STATUSES.length && LEGACY_STATUSES.every((value) => statuses.includes(value));
  return {
...
function ChannelFilterFields({ form, setForm }: { form: FormState; setForm: (value: FormState) => void }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 p-4">
      ...
      <p className="mt-1 text-xs leading-5 text-slate-400">到期类提醒按每日计划发送；低余额等 Condition 在触发后主动发送。两者都会遵守这里的来源和状态筛选。</p>
...
export default function NotificationsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="rounded-xl border border-slate-100 bg-white px-4 py-3 text-xs leading-5 text-slate-400"><Clock3 className="mr-1 inline h-3.5 w-3.5" />每日到期提醒与实时 Condition 共用通知渠道、模板和发送历史；Condition 恢复后会自动停止本轮外部提醒。</div>
    </div>
  );
}
