...
- 完整 JSON 导出、本地备份、恢复前安全备份及保留策略；设备、删除概要与 eSIM 凭据均可随可移植备份跨实例恢复
...
`.credential-secret` 用于解密本实例保存的 eSIM 激活凭据，不应单独删除。SIMKeeper 的可移植 JSON 备份会携带跨实例恢复所需的凭据密钥，因此备份文件本身也属于高敏感数据，请按密码文件同等级别妥善保管。
